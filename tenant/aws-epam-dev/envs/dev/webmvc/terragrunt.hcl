...
locals {
  ...
  name = "webmvc"
  image_name = "winshiftq/webmvc:linux-terraform"
  cloudwatch_log_group_name = "/aws/ecs/eshop/webmvc"
...
inputs = {
  ...
  container_definitions = templatefile("container_definitions.json", {
    ...
    cloudwatch_log_group_name = local.cloudwatch_log_group_name
    purchase_url = dependency.ecs.outputs.apigatewayv2_api_api_endpoint
    identity_url = dependency.ecs.outputs.apigatewayv2_api_api_endpoint
    signal_rhub_url = dependency.ecs.outputs.apigatewayv2_api_api_endpoint
  })
}
